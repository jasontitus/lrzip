	   ...
	   i64 offset, double pct_base, double pct_multiple)
{
	 ...
	 * end can find. Only attempted when the whole chunk is mapped (no
	 * sliding mmap) and the back end benefits from it. The trade off is
	 * that identical code duplicated at different offsets converts
	 * differently, so the trial demands a clear win first. */
	st->chunk_md5_done = false;
	control->chunk_filter = LRZ_FILTER_NONE;
	if ((LZMA_COMPRESS || ZSTD_COMPRESS) && st->mmap_size == st->chunk_size &&
	    st->chunk_size >= (1 << 20)) {
		int kind = lrz_chunk_filter_pick(control, sb->buf_low, st->chunk_size);
