...
int lrz_filter_trial(rzip_control *control, uchar *sample_area, i64 avail, bool branch_only)
{
	...
	int best = LRZ_FILTER_NONE, last, ret, i;
	int trial_level, trial_dict;
	...
	/* Chunk (branch_only) trials guard a whole chunk, so probe harder:
	 * branch conversion's gains come from repeats at large distances
	 * that a small fast dictionary cannot see. */
	if (branch_only) {
		sample = MIN((size_t)avail, (size_t)(1 << 22));
		trial_level = 2;
		trial_dict = 1 << 22;
	} else {
	...
	ret = LzmaCompress(out, &plain_len, pristine, sample, props, &prop_size,
			   trial_level, trial_dict, -1, -1, -1, -1, 1);
	...
	last = branch_only ? LRZ_FILTER_ARM64 : LRZ_FILTER_MAX;
	for (i = LRZ_FILTER_X86; i <= last; i++) {
